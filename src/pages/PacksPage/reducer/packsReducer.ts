...
const randomNumber = (count: number): number => Math.floor(Math.random() * (count - 1));
const initState = {
  isInitialized: false,
...
const slice = createSlice({
  ...
  reducers: {
    ...
    setSearchPackNameValue(
      state,
      action: PayloadAction<{ searchPackNameValue: string }>,
    ) {
    ...
    setSelectedPack(
      state,
      action: PayloadAction<{ cardsPack_id: string; cardsCount: number; name: string }>,
    ) {
  ...
  extraReducers: builder => {
    ...
    builder.addCase(getTrainingCards.pending, state => {
      state.selectedPack.isInitialized = false;
      state.selectedPack.status = 'loading';
    });
    builder.addCase(setCardNumber.pending, state => {
      state.selectedPack.status = 'loading';
    });
...
>(
  ...
  async (requestModel: RequestModelType, { getState, dispatch, rejectWithValue }) => {
    ...
    const userId = (getState() as AppRootStateType).auth.authData._id;
    const requestParams = {
      page: packs.data.page,
      pageCount: packs.data.pageCount,
      user_id: packs.isAuthUserPacks ? userId : null,
...
export const editPackName = createAsyncThunk(
  ...
  async (cardsPack: { _id: string; name: string }, { dispatch, rejectWithValue }) => {
    dispatch(setPacksStatus({ packsStatus: 'loading' }));
    try {
      const res = await packsAPI.updatePackName({ ...cardsPack, private: false });
...
export const getTrainingCards = createAsyncThunk(
  ...
  async (_, { rejectWithValue, getState }) => {
    const { params } = (getState() as AppRootStateType).packs.selectedPack;
    const reqParams = {
