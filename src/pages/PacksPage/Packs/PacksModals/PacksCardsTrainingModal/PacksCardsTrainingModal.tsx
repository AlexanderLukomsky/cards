...
const cardNumber = (state: AppRootStateType) => state.packs.selectedPack.cardNumber
...
export const PacksCardsTrainingModal: React.FC<PropsType> = React.memo(({ onCloseModal, isOpen }) => {
   const dispatch = useAppDispatch()
   const [selectedGrade, setSelectedGrade] = useState(0)
   const { selectedPack } = useAppSelector(state => state.packs)
   useEffect(() => {
      if (selectedPack.params.cardsCount > 0) { dispatch(getTrainingCards()) }
   }, [])
   ...
   const nextCard = () => {
      ...
      const cardNumber = randomNum !== selectedPack.cardNumber ? randomNum : getRandomNumber(selectedPack.params.cardsCount)
      dispatch(setCardNumber({ cardNumber }))
   }
   const onChangeRate = (event: React.ChangeEvent<HTMLInputElement>, value: string) => {
         ...
         cancelBtnProps={
            {
               ...
               disabled: selectedPack.status === 'loading' || selectedPack.params.cardsCount === 0,
      ...
      >
         {selectedPack.params.cardsCount === 0 ?
            <div className='packs__training-modal__text'>
               <p><span> Question:</span>"no question"</p>
               <p><span>Answer:</span>"no answer"</p>
            </div> :
               ...
               <div className='packs__training-modal__form'>
                  <FormControl >
                     ...
                     <RadioGroup onChange={onChangeRate} >
                        <FormControlLabel checked={selectedGrade === 1} value={1} control={<Radio color="error" />} label="Did not know" />
                        <FormControlLabel checked={selectedGrade === 2} value={2} control={<Radio color="error" />} label="Forgot" />
                        <FormControlLabel checked={selectedGrade === 3} value={3} control={<Radio color="primary" />} label="A lot of thought" />
                        <FormControlLabel checked={selectedGrade === 4} value={4} control={<Radio color="success" />} label="Confused" />
                        <FormControlLabel checked={selectedGrade === 5} value={5} control={<Radio color="success" />} label="Knew the answer" />
                     </RadioGroup>
                  </FormControl>
               </div>
            </div>
