...
export const Packs: React.FC<PropsType> = (
   { packs, onChangePage, title, page, isInitialized, pageCount, cardPacksTotalCount, onSearchPacks, onChagePageCount, progressStatus }
) => {
   return (
      <div className="packs-page__packs packs">
         ...
         {!isInitialized ? <AppCircularProgress /> : <PacksList packs={packs} progressStatus={progressStatus} />}
         <div className="packs__footer">
            <PacksPagination onChange={onChangePage} page={page} pageCount={pageCount} totalCount={cardPacksTotalCount} />
            <CustomSelect items={[5, 10, 15]} value={pageCount} onChange={onChagePageCount} />
         </div>
