...
export const AppSnackBar = React.memo(() => {
   ...
   return (
      <Stack spacing={2} sx={{ minWidth: '360px' }}>
         <Snackbar open={isOpen} onClose={handleClose} autoHideDuration={4000} style={{ left: '50%', transform: 'translateX(-50%)' }}>
            <Alert onClose={handleClose} severity={appStatus === 'failed' ? 'error' : 'success'} sx={{ width: '100%' }}>
               {appMessage}
            </Alert>
         </Snackbar>
      </Stack>
