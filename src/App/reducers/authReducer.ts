...
export const editedProfile = createAsyncThunk<
  UpdateUserType,
  EditProfileDataType,
  { rejectValue: { error: string }; dispatch: AppDispatchType }
>('auth/edit-profile', async (data: EditProfileDataType, { dispatch, rejectWithValue }) =>
  authThunkHandler(authAPI.updateUser(data), dispatch, rejectWithValue),
);
...
export const logout = createAsyncThunk<
  ...
  undefined,
  { rejectValue: { error: string }; dispatch: AppDispatchType }
>('auth/logout', (_, { dispatch, rejectWithValue }) =>
  authThunkHandler(authAPI.logout(), dispatch, rejectWithValue),
);
export const registration = createAsyncThunk<
  {},
  RegisterDataType,
  { rejectValue: { error: string }; dispatch: AppDispatchType }
>('auth/registration', (data: RegisterDataType, { dispatch, rejectWithValue }) =>
  authThunkHandler(authAPI.registration(data), dispatch, rejectWithValue),
);
export const setLogin = createAsyncThunk<
  LoginResponseType,
  AuthDataType,
  { rejectValue: { error: string } }
>('auth/login', async (data: AuthDataType, { dispatch, rejectWithValue }) => {
  try {
    ...
    const error: string = (err as AxiosError).response
...
export const forgotPassword = createAsyncThunk<
  {},
  ForgotPasswordDataType,
  { dispatch: AppDispatchType; rejectValue: { error: string } }
>(
  'auth/password-recovery',
  (data: ForgotPasswordDataType, { dispatch, rejectWithValue }) =>
    authThunkHandler(authAPI.forgotPassword(data), dispatch, rejectWithValue),
);
export const setNewPassword = createAsyncThunk<
  {},
  NewPasswordDataType,
  { dispatch: AppDispatchType; rejectValue: { error: string } }
>('auth/set-new-password', (data: NewPasswordDataType, { dispatch, rejectWithValue }) =>
  authThunkHandler(authAPI.newPassword(data), dispatch, rejectWithValue),
);
const authThunkHandler = async (
  asyncFn: Promise<AxiosResponse<LoginResponseType | UpdateUserType>>,
