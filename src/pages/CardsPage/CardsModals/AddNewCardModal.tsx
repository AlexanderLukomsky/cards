...
export const AddNewCardModal: React.FC<PropsType> = React.memo(
  ({ cardsPack_id }) => {
    ...
    const onAddNewCard = async () => {
      ...
      if (question.trim().length > 30) {
        setError((err) => ({
          ...err,
          questionError: "maximum question length 30",
        }));
        return;
      } else if (answer.trim().length > 30) {
        setError((err) => ({
          ...err,
          answernError: "maximum answer length 30",
        }));
        return;
      }
      const action = await dispatch(
        addNewCard({ question, answer, cardsPack_id })
      );
        ...
        {openModal && (
          <ModalContainer
            ...
            secondBtnProps={{
              ...
              disabled: packsStatus === "loading",
            }}
          >
            {packsStatus === "loading" && <AppCircularProgress />}
            <TextField
              variant="standard"
              className={errors.questionError ? "error" : ""}
              label={`${
                errors.questionError ? errors.questionError : "Question"
              }`}
              value={question}
            ...
            <TextField
              variant="standard"
              className={errors.answernError ? "error" : ""}
